...
class HorseRaceApp:
    VALID_BREEDS = {"Thoroughbred": Thoroughbred, "Appaloosa": Appaloosa}
    ...
    def add_horse(self, horse_type: str, horse_name: str, horse_speed: int):
        ...
        if horse_type in self.VALID_BREEDS:
            self.horses.append(self.VALID_BREEDS[horse_type](horse_name, horse_speed))
    ...
    def start_horse_race(self, race_type: str):
        try:
            current_race = next(filter(lambda r: r.race_type == race_type, self.horse_races))
        except StopIteration:
            raise Exception(f"Race {race_type} could not be found!")
